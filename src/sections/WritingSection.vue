  ...
  <section id="writing" class="chapter writing" data-theme="ink" aria-labelledby="writing-title">
    <div class="container">
      <div class="chapter__head mono">
        ...
        <a v-if="data.medium" :href="data.medium" target="_blank" rel="noopener noreferrer" class="link-ul writing__medium">Medium</a>
      </div>
      ...
      <div class="writing__grid">
        ...
        <div class="writing__feed" aria-live="polite">
          <ul v-if="data.items?.length" class="feed">
            <li v-for="item in data.items" :key="item.title" class="feed__row">
              <div class="feed__link feed__link--static">
                <span class="feed__date mono muted">{{ item.source }}</span>
                <span class="feed__title">{{ item.title }}</span>
                <span class="feed__tags mono muted">Certification</span>
                <CheckCircle class="feed__arrow" :size="18" :stroke-width="1.75" aria-hidden="true" />
              </div>
            </li>
          </ul>

          <ul v-else-if="loading && !posts.length" class="feed feed--skeleton" aria-label="Loading articles">
...
import { ArrowUpRight, CheckCircle } from 'lucide-vue-next';
